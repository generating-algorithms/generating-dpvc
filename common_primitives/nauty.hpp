...
using _NautyCanonicalRep = std::pair<int, _NautyCanonicalRepEdges>;

_NautyCanonicalRep make_empty_nauty_canonical_rep() {
    return {0, _NautyCanonicalRepEdges()};
}
...
struct _Nauty {
    ...
    _NautyCanonicalRep get_canonical_rep(int n, const vector<pair<int,int>> & edges) {
        assert(n<=MAXN);
        ...
        for(int i = 0; i < n; ++i)__nauty_relab[__nauty_lab[i]]=i;
        _NautyCanonicalRepEdges canonical_rep_edges;
        for(pair<int,int> e : edges) {
            int i = __nauty_relab[e.first];
            int j = __nauty_relab[e.second];
            if(i>j)std::swap(i,j);
