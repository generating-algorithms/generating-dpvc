...
int main(int argc, char ** argv) {
    ...
    for(int generation = 1; generation <= GENERATIONS; ++generation) {
        ...
        while(something_changed) {
            ...
            if(round!=1) {
                parallel_filter_graphs_containing_forbidden_induced_subgraph(
                    current_bumpy_graph_expansions, local_forbidden_induced_subgraphs, nts);
            }
            ...
            for(int g_idx=0; g_idx < current_bumpy_graphs.size(); ++g_idx) {
                ...
                if(br.bf <= DPVC_BF) {
                    ...
                    forbidden_induced_subgraphs.add(g.canonical_rep, g.n);
                    local_forbidden_induced_subgraphs.add(g.canonical_rep, g.n);
        ...
        // The generation ended. If this was the last iteration, dont bother with propagation and gathering.
    ...
    std::ofstream stat_br_ofs ("stats.out", std::ofstream::out);
    stat_br_ofs<<"{"<<endl;
    ...
    std::ofstream baf_br_ofs ("not_solved_branching_rules.out", std::ofstream::out);
    for(const auto & br : not_solved_branching_rules){
        baf_br_ofs<<export_branching_rule_to_jsonl(br)<<endl;
    }
    baf_br_ofs.close();
    std::ofstream br_ofs ("branching_rules.out", std::ofstream::out);
