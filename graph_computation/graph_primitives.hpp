...
struct _Graph {
    ...
    _Graph(int n, const vector<pair<int,int>> & edges, const vector<int> & red_vertices, _Nauty & nt)
        : n(n), canonical_rep(nt.get_canonical_rep(n, edges)) {
    ...
    _Graph(int n, _GraphEdges edges, _NautyCanonicalRep canonical_rep, int red_vertices_mask)
        : n(n), edges(edges), canonical_rep(canonical_rep), red_vertices_mask(red_vertices_mask) {}
    ...
    bool operator < (const _Graph & og) const {
        return canonical_rep < og.canonical_rep;
    }
